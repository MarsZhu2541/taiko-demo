...
step("Open admintool", async function () {
    await goto("http://localhost:8081/admintool/signin");
});

step("Login with username <username> and password <password>", async function (username, password) {
    await write(username, textBox({'name':'username'}));
    await write(password, textBox({'name':'password'}));
    await click("Login") 
    await click("Choose")
});

step("Search VIN <vin> in Vehicle Monitoring", async function (vin) {
    await click("Vehicle Monitoring")
    await write(vin, textBox({'name':'vin'}));
    await click("Search");
});

step("Assert Model Year equals <modelYear>", async function (year) {
    assert.equal(await ($(`//*[@id="vehicleBasicDataTable"]/tbody/tr[8]/td/span`)).text(), year);
});

step("Logout admintool", async function () {
    await click("Logout");
});